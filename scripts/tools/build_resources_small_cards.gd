...
const SOURCE_TEXTURE: Texture2D = preload("res://assets/graphics/8BitDeckAssets_GB.png")
...
const CARD_START_POS: Vector2 = Vector2i(14, 12)
const CARD_DISTANCE: Vector2 = Vector2i(35, 47)
const CARD_SIZE: Vector2 = Vector2i(33, 45)


func _run() -> void:
	print("Building resources")
	_build_resources_for_suite(CardInfo.Suit.HEARTS, 1, 0)
	_build_resources_for_suite(CardInfo.Suit.CLUBS, 1, 1)
	_build_resources_for_suite(CardInfo.Suit.DIAMONDS, 1, 2)
	_build_resources_for_suite(CardInfo.Suit.SPADES, 1, 3)
	_build_resources_for_card(CardInfo.Suit.RED, CardInfo.Rank.JOKER, 0, 2)
	_build_resources_for_card(CardInfo.Suit.BLACK, CardInfo.Rank.JOKER, 0, 3)
	_build_card_texture_resource(CardInfo.Suit.COLORLESS, CardInfo.Rank.BACK, 0, 0)
...
func _build_resources_for_suite(suit: CardInfo.Suit, col: int, row: int) -> void:
	for i in range(0, 13):
		...
		if rank==13:
			rank=0
		_build_resources_for_card(suit, rank, col+i, row)
